...
export default function ChannelSidebar() {
  ...
  return (
    <aside id="channel-sidebar" className="glass flex flex-col w-60 min-w-[240px] z-10">
      {/* Server Header */}
      <button id="server-header-btn" className="flex items-center justify-between px-4 h-12 border-b border-border-subtle hover:bg-surface-light/50 transition-colors cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4">
        {categories.map((cat) => (
          <div key={cat.name}>
            <button onClick={() => toggle(cat.name)} className="flex items-center gap-0.5 px-1 mb-1 w-full text-[11px] font-semibold tracking-wider text-text-muted hover:text-text-secondary transition-colors cursor-pointer uppercase">
            ...
            {!collapsed.has(cat.name) && cat.channels.map((ch) => (
              <div key={ch.id}>
                <button id={`channel-${ch.id}-btn`} onClick={() => setActiveChannel(ch.id)}
                  className={`group flex items-center gap-2 w-full px-2 py-1.5 rounded-md text-sm transition-all duration-150 cursor-pointer ${activeChannel === ch.id ? "bg-surface-lighter/70 text-text-primary" : "text-text-muted hover:text-text-secondary hover:bg-surface-light/40"}`}>
                  {ch.type === "text" ? (ch.locked ? <Lock size={16} className="shrink-0 text-text-muted" /> : <Hash size={16} className="shrink-0" />) : <Volume2 size={16} className="shrink-0" />}
                ...
                {ch.type === "voice" && ch.users?.map((u) => (
                  <div key={u.name} className="flex items-center gap-2 ml-6 mt-0.5 px-2 py-1 rounded text-xs text-text-secondary">
                    <div className="relative">
                      <div className="w-6 h-6 rounded-full bg-surface-lighter flex items-center justify-center text-[10px] font-semibold text-violet-glow">{u.avatar}</div>
      ...
      <div id="user-panel" className="flex items-center gap-2 px-2 py-2 border-t border-border-subtle bg-surface-darkest/60">
        <div className="relative">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-electric to-violet-deep flex items-center justify-center text-xs font-bold text-white">JL</div>
        ...
        <div className="flex items-center gap-0.5">
          <button id="user-mic-btn" className="p-1.5 rounded hover:bg-surface-light/50 text-text-secondary hover:text-text-primary transition-colors cursor-pointer"><Mic size={14} /></button>
          <button id="user-headphones-btn" className="p-1.5 rounded hover:bg-surface-light/50 text-text-secondary hover:text-text-primary transition-colors cursor-pointer"><Headphones size={14} /></button>
          <button id="user-settings-btn" className="p-1.5 rounded hover:bg-surface-light/50 text-text-secondary hover:text-text-primary transition-colors cursor-pointer"><Settings size={14} /></button>
        </div>
      </div>
    </aside>
  );
